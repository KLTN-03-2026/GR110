...
export const updateBlockBackgroundAPI = async ({ backgroundId }) => {
  const response = await authorizeAdminAxiosInstance.patch(`${API_ROOT}/v1/admin/backgrounds/block/${backgroundId}`)
  toast.success('Background change status successfully!')
  return response.data.metadata
}

export const updateAdminBackgroundAPI = async ({ _id , backgroundData }) => {
  const response = await authorizeAdminAxiosInstance.post(`${API_ROOT}/v1/admin/backgrounds/update/${_id}`, backgroundData)
  toast.success('Admin background updated successfully!')
  return response.data.metadata
}

export const createAdminBackgroundAPI = async ({ backgroundData }) => {
  const response = await authorizeAdminAxiosInstance.post(`${API_ROOT}/v1/admin/backgrounds`, backgroundData)
  toast.success('Admin background created successfully!')
  return response.data.metadata
}

export const deleteAdminBackgroundAPI = async ({ _id }) => {
  const response = await authorizeAdminAxiosInstance.delete(`${API_ROOT}/v1/admin/backgrounds/delete/${_id}`)
